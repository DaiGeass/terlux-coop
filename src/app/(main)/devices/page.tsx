...
export default function DevicesPage() {
  ...
  const remove = async (id: string) => {
    if (!confirm(t("¿Eliminar este dispositivo?"))) return;
  ...
  return (
    <div className="space-y-5">
      <div className="page-header">
        <div>
          <h1 className="page-title">{t("Gestión de dispositivos (MDM)")}</h1>
          <p className="page-subtitle">{t("Inventario, asignación y seguimiento de equipos de la empresa")}</p>
        </div>
        <button onClick={openNew} className="btn btn-primary gap-2"><Plus size={16} /> {t("Nuevo dispositivo")}</button>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">{t("Total")}</p>
          <p className="text-2xl font-bold mt-1">{stats.total}</p>
        </div>
        <div className="glass-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">{t("Disponibles")}</p>
          <p className="text-2xl font-bold mt-1 text-emerald-500">{stats.available}</p>
        </div>
        <div className="glass-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">{t("Asignados")}</p>
          <p className="text-2xl font-bold mt-1 text-blue-500">{stats.assigned}</p>
        </div>
        <div className="glass-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">{t("Mantenimiento")}</p>
      ...
      <div className="glass-card p-3 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("Buscar por nombre, marca, modelo o nº serie…")}
        ...
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="form-select text-sm w-40">
          ...
          {TYPES.map((x) => <option key={x.id} value={x.id}>{t(x.label)}</option>)}
        </select>
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="form-select text-sm w-40">
          ...
          {Object.entries(STATUS_META).map(([k, v]) => <option key={k} value={k}>{t(v.label)}</option>)}
        </select>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[10px] uppercase text-muted-foreground border-b border-border/30">
                <th className="p-3">{t("Dispositivo")}</th>
                <th className="p-3">{t("Sistema")}</th>
                <th className="p-3">{t("Red")}</th>
                <th className="p-3">{t("Asignado a")}</th>
                <th className="p-3">{t("Ubicación")}</th>
            ...
            <tbody>
                ...
                return (
                  <tr key={d.id} className="border-b border-border/15 hover:bg-accent/30">
                    ...
                    <td className="p-3">
                      ...
                      ) : <span className="text-muted-foreground text-xs">{t("Sin asignar")}</span>}
                    </td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      <span className="inline-flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full font-medium"
                        ...
                        <StatusIcon size={11} /> {t(meta.label)}
                      </span>
                    </td>
                    <td className="p-3">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="p-12 text-center text-muted-foreground">
                  {loading ? t("Cargando…") : t("No hay dispositivos. Haz clic en 'Nuevo dispositivo' para empezar.")}
                </td></tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="glass-modal rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 glass-navbar px-6 py-4 flex items-center justify-between z-10 border-b border-border/30">
              <h3 className="font-semibold">{editing ? t("Editar dispositivo") : t("Nuevo dispositivo")}</h3>
              <button onClick={() => setShowForm(false)}><XCircle size={18} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2"><label className="text-xs text-muted-foreground">{t("Nombre")}</label>
                  <input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t("Ej: Laptop María 2024")} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Tipo")}</label>
                  <select className="form-select" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                    {TYPES.map((x) => <option key={x.id} value={x.id}>{t(x.label)}</option>)}
                  </select></div>
                <div><label className="text-xs text-muted-foreground">{t("Estado")}</label>
                  <select className="form-select" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                    {Object.entries(STATUS_META).map(([k, v]) => <option key={k} value={k}>{t(v.label)}</option>)}
                  </select></div>
                <div><label className="text-xs text-muted-foreground">{t("Marca")}</label>
                  <input className="form-input" value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} placeholder={t("Dell, Apple, HP…")} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Modelo")}</label>
                  <input className="form-input" value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} placeholder={t("XPS 15, MacBook Pro…")} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Nº Serie")}</label>
                  <input className="form-input font-mono" value={form.serialNumber} onChange={(e) => setForm({ ...form, serialNumber: e.target.value })} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Ubicación")}</label>
                  <input className="form-input" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder={t("Oficina Madrid, Almacén…")} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Sistema operativo")}</label>
                  <input className="form-input" value={form.os} onChange={(e) => setForm({ ...form, os: e.target.value })} placeholder={t("Windows 11, macOS Sonoma…")} /></div>
                <div><label className="text-xs text-muted-foreground">{t("Versión OS")}</label>
                  <input className="form-input" value={form.osVersion} onChange={(e) => setForm({ ...form, osVersion: e.target.value })} /></div>
                <div><label className="text-xs text-muted-foreground">{t("IP")}</label>
                ...
                <div><label className="text-xs text-muted-foreground">{t("MAC")}</label>
            ...
            <div className="sticky bottom-0 glass-navbar px-6 py-4 flex justify-end gap-2 border-t border-border/30">
              ...
              <button className="btn btn-primary gap-2" onClick={save}>{t("Guardar dispositivo")}</button>
            </div>
          </div>
        </div>
      )}
