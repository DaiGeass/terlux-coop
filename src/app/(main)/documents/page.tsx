    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Contratos, políticas, manuales y documentación corporativa</p>
        </div>
        <button onClick={openNew} className="btn btn-primary gap-2"><Plus size={16} /> Nuevo documento</button>
      </div>
        ...
        {filtered.map((d) => {
          ...
          return (
            <div key={d.id} className="glass-card p-4 group hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: st.color + "20", color: st.color }}>{st.label}</span>
              ...
              <p className="text-xs text-muted-foreground mt-3 line-clamp-2">{d.content || "Sin contenido"}</p>
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="glass-modal rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border/30">
              <h3 className="font-semibold">{editing === "new" ? "Nuevo documento" : "Editar documento"}</h3>
              <button onClick={() => setEditing(null)}><X size={18} /></button>
            </div>
            <div className="p-6 space-y-4 overflow-y-auto">
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Título del documento" className="form-input text-base font-medium" />
              ...
              <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={16}
                placeholder="Contenido del documento…" className="form-textarea font-mono text-sm leading-relaxed" />
            </div>
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-border/30">
              ...
              <button className="btn btn-primary gap-2" onClick={save}><Save size={15} /> Guardar</button>
            </div>
          </div>
        </div>
      )}
